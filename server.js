...
function connectAccount(username, password, socketIndex = null) {
  return new Promise((resolve, reject) => {
    ...
    socket.on("open", () => {
      // API requires auth.required before developer.login.
    });
...
app.post("/api/batch-action", async (req, res) => {
  const { action, room, targetUsername, message } = req.body || {};
  // IMPORTANT: batch commands operate on the backend's authoritative session map.
  // The browser's cached sessionIds may be stale after logout/session.replaced.
  const ids = [...sessions.keys()];
  if (!ids.length || !action) return res.status(400).json({ ok: false, error: "Tidak ada session aktif atau action tidak lengkap." });

  const cleanRoom = String(room || "").trim();

  if (action === "join") {
    if (!cleanRoom) return res.status(400).json({ ok: false, error: "Room wajib diisi." });
    const results = await Promise.all(ids.map(async (sessionId) => {
      const account = sessions.get(sessionId);
      if (!account) return { sessionId, ok: false, error: "Session sudah tidak aktif." };
      if (!account.permissions.includes("rooms.join")) return { sessionId, ok: false, error: "Permission rooms.join tidak tersedia." };
      if (account.pendingRoomAction) return { sessionId, ok: false, error: "Perintah room sebelumnya masih diproses." };
      if (account.joinedRoom) return { sessionId, ok: false, error: `Sudah berada di room ${account.joinedRoom}.` };
      account.pendingRoomAction = { type: "join", room: normalizeRoomName(cleanRoom) };
      try {
        const waiter = waitForJoin(sessionId, cleanRoom, 10000);
        send(sessionId, { type: "room.join", room: cleanRoom });
        const event = await waiter;
        return { sessionId, ok: true, username: account.username, event };
      } catch (e) {
        return { sessionId, ok: false, username: account.username, error: safeError(e), event: e?.event || null };
      } finally {
        const current = sessions.get(sessionId);
        if (current) current.pendingRoomAction = null;
      }
    }));
    const success = results.filter(x => x.ok).length;
    return res.json({ ok: success === results.length, action, room: cleanRoom, sent: ids.length, success, total: results.length, results });
  }
  ...
  if (action === "leave") {
    ...
    const results = await Promise.all(ids.map(async (sessionId) => {
      const account = sessions.get(sessionId);
      if (!account) return { sessionId, ok: false, error: "Session sudah tidak aktif." };
      if (!account.permissions.includes("rooms.leave")) return { sessionId, ok: false, error: "Permission rooms.leave tidak tersedia." };
      if (account.pendingRoomAction) return { sessionId, ok: false, error: "Perintah room sebelumnya masih diproses." };
      account.pendingRoomAction = { type: "leave", room: normalizeRoomName(cleanRoom) };
      try {
        const waiter = waitForLeave(sessionId, cleanRoom, 10000);
        send(sessionId, { type: "room.leave", room: cleanRoom });
        const event = await waiter;
        return { sessionId, ok: true, username: account.username, event };
      } catch (e) {
        return { sessionId, ok: false, username: account.username, error: safeError(e), event: e?.event || null };
  ...
  if (action === "participants") {
    ...
    const results = await Promise.all(ids.map(async (sessionId) => {
      const account = sessions.get(sessionId);
      if (!account) return { sessionId, ok: false, error: "Session sudah tidak aktif." };
      if (!account.permissions.includes("rooms.read")) return { sessionId, ok: false, error: "Permission rooms.read tidak tersedia." };
      if (normalizeRoomName(account.joinedRoom) !== normalizeRoomName(cleanRoom)) return { sessionId, ok: false, username: account.username, error: "Akun belum join room ini." };
      try {
        const waiter = waitForParticipants(sessionId, cleanRoom, 10000);
        send(sessionId, { type: "room.participants", room: cleanRoom });
        const event = await waiter;
        return { sessionId, ok: true, username: account.username, event };
      } catch (e) {
        return { sessionId, ok: false, username: account.username, error: safeError(e), event: e?.event || null };
  ...
  else if (action === "kick") { if (!cleanRoom || !targetUsername) return res.status(400).json({ ok: false, error: "Room dan target wajib diisi." }); payload = { type: "room.kick", room: cleanRoom, target_username: targetUsername }; }
  else if (action === "message") { if (!cleanRoom || !message) return res.status(400).json({ ok: false, error: "Room dan pesan wajib diisi." }); payload = { type: "room.send_message", room: cleanRoom, message }; }
  else return res.status(400).json({ ok: false, error: "Action tidak dikenal." });

  const results = ids.map(sessionId => {
    try { send(sessionId, payload); return { sessionId, ok: true }; }
    catch (e) { return { sessionId, ok: false, error: safeError(e) }; }
  });
  res.json({ ok: results.every(x => x.ok), action, sent: results.filter(x => x.ok).length, total: results.length, results });
});
...
app.post("/api/logout", (req, res) => {
  ...
  if (!id) return res.status(400).json({ ok: false, error: "sessionId wajib diisi." });
  const closed = closeSession(id, "logout");
  res.json({ ok: closed, closed: closed ? 1 : 0, remaining: sessions.size });
});
...
app.post("/api/logout-batch", (_req, res) => {
  ...
  // closeSession removes from the authoritative map synchronously.
  res.json({ ok: sessions.size === 0, closed, requested: ids.length, remaining: sessions.size });
});
